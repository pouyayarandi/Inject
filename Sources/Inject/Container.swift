...
public final class AppContainer {
    ...
    public func resolve<T>(type: T.Type) -> T {
        ...
        // Use sync for reading and barrier flag for writing to ensure thread safety
        return queue.sync {
            ...
            if singletonTypes.contains(typeId) {
                // Use barrier flag for writing to ensure exclusive access
                queue.async(flags: .barrier) {
                    self.instances[typeId] = instance
                }
            }
...
extension AppContainer {
    public func register<T>(_ type: T.Type, isSingleton: Bool = false, factory: @escaping () -> T) {
        ...
        queue.async(flags: .barrier) {
            ...
            if isSingleton {
                self.singletonTypes.insert(ObjectIdentifier(type))
            }
        }
    }
} 
